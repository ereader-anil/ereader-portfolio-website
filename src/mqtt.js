...
function getMqttClient() {
  ...
  const options = {
    ...
    clientId: `${cfg.clientIdPrefix || 'station-manager'}-${Date.now()}`,
    clean: true,
...
export async function publishAws(topic, payload, qos = null){
  const cfg = loadMqttSettings()
  if(!cfg.broker){
  ...
  return new Promise((resolve, reject) => {
    if (!client.connected) {
      client.on('connect', () => {
        client.publish(topic, JSON.stringify(payload), { qos: qos !== null ? qos : (cfg.qos || 1), retain: cfg.retain || false }, (err) => {
          if (err) reject(err)
          else resolve()
        })
      })
